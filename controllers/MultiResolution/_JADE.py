...
class JADE:
    ...
    def search(self):
        ...
        self.archive_X = [X.clone().cpu()]
        self.archive_f = [fX.clone().cpu()]
        ...
        while evals < self.max_evals - fX.shape[0]:
            ...
            self.archive_X.append(U.clone().cpu())
            self.archive_f.append(fU.clone().cpu())
        ...
        archive_X_np = torch.cat(self.archive_X, dim=0).numpy()
        archive_f_np = torch.cat(self.archive_f, dim=0).numpy()


        return X[f_best_indx].cpu().numpy(), X.cpu().numpy(), fX.cpu().numpy(), archive_X_np, archive_f_np

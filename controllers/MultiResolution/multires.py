...
for run_id in range(1, n_runs + 1):
    ...
    while current_step < total_steps:
        ...
        #if last_price_seq is None or not np.array_equal(current_price_seq, last_price_seq):
        ...
        if exploratory_soc_horizon > 0 and steps_left >= BLOCK * exploratory_soc_horizon:
            ...
            print(f"Exploratory terminal SoC target: {terminal_soc:.4f}")
            print(f"Exploratory terminal TMedian target: {terminal_tmedian:.4f}")
            fmu_term.freeInstance()
        else:
            terminal_soc = 0
        ...
        if current_step < BLOCK:
            terminal_soc = 0
            terminal_tmedian = None
            print ('first day terminal soc', terminal_soc)        
        ...
        if elite_warm_start and current_step >= BLOCK:
            if elite_pool.size == 0:
                ...
                #if elite_pool.shape[0] > swarm_HF:
                #    elite_pool = elite_pool[-swarm_HF:]

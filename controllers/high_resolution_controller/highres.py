...
def run_horizon(u_mat, state0, cost0, t0, terminal_soc_target, steps_seconds):
    fmu, vr = make_fmu()
    try:
        ...
        soc_dev_penalty = 5000 * abs(soc - terminal_soc_target)  # weight can be tuned
        
        return delta_cost + pen + soc_dev_penalty
    finally:
        fmu.freeInstance()
    ...
    while current_step < total_steps:
        ...
        if exploratory_soc_horizon > 0 and steps_left >= BLOCK * exploratory_soc_horizon:
            ...
            def _one_cost_low_res(x, state, c0, t0, LOW_RES_STEP_SEC):
                ...
                except FMICallException: return 1e6
            ...
            # Only first day's worth of low-res steps
        ...
        def _one_cost(x, state, c0, t0, term_soc, STEPS_SECONDS):
            try:
                u = np.column_stack((x[:H], x[H:]))
                return run_horizon(u, state, c0, t0, term_soc, STEPS_SECONDS)
        ...
        def obj_batch(X_batch):
            with mp.Pool(processes=os.cpu_count()) as pool:
                jobs = [(x, STATE0, cost0, T0_snap, terminal_soc, STEPS_SECONDS) for x in X_batch]

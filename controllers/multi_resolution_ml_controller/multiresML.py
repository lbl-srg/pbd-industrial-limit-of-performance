...
for run_id in range(1, n_runs + 1):
    ...
    rf_model = RandomForestRegressor(n_estimators=200, random_state=111)  # Use Random Forest
    clf_features = []
    terminal_soc_values = []
    ...
    with open(exploration_decision_file, "w") as f:
        f.write("current_step\tforce_exploration\treason\tmean_prediction\tterminal_soc\tucb\tbudget\n")
    ...
    while current_step < total_steps:
        ...
        feat_vec = np.concatenate([
            get_48h_price_projection(current_step),
            get_48h_weather_projection(current_step)
        ])
        ...
        if seasonal_warmup_steps > 0:
        ...
        elif (current_step // (24 * 3600 / STEPS_SECONDS)) % 5 == 0:
            force_exploration = True
            ...
            if len(clf_features) >= seasonal_warmup_steps:
                rf_model.fit(np.array(clf_features), terminal_soc_values)
                predictions = np.array([tree.predict(feat_vec.reshape(1, -1)) 
                                         for tree in rf_model.estimators_])
                mean_prediction = np.mean(predictions)
                std_prediction = np.std(predictions)
                ucb = mean_prediction + 2 * std_prediction
            
                prediction_log.append((current_step, mean_prediction, std_prediction, ucb))
                print(f"Mean: {mean_prediction:.6f}, Std: {std_prediction:.6f}, UCB: {ucb:.6f}")
            
                pred_file = os.path.join(run_results_dir, "exploration_predictions.txt")
                with open(pred_file, 'a') as f:
                    f.write(f"{current_step}\t{mean_prediction}\t{std_prediction}\t{ucb}\n")
                if ucb >= 0.01:
                    force_exploration = True
                    reason = "RF_high_ucb"
                else:
                    ...
                    terminal_soc = mean_prediction
            else:
                prediction_log.append((current_step, None, None, None))
                force_exploration = True
                reason = "missing_features"
        ...
        with open(exploration_decision_file, "a") as f:
            f.write(
                ...
                f"{reason}\t"
                f"{-1 if mean_prediction is None else mean_prediction:.6f}\t"
                f"{-1 if terminal_soc is None else np.abs(terminal_soc):.6f}\t"
                f"{-1 if ucb is None else ucb:.6f}\t"
                f"{expl_budget_log}\n"
            )

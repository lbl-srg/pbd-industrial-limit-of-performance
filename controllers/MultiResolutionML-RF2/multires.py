...
import copy
from sklearn.ensemble import RandomForestRegressor  # Use Random Forest for regressor
import joblib
import time



def get_48h_price_projection(current_step):
    return price_values[current_step:current_step + 96][::4]

def get_48h_weather_projection(current_step):
    return weather_values[current_step:current_step + 96][::4]
...
def get_24h_weather_projection(current_step):
    return weather_values[current_step:current_step + 48]
...
def run_horizon(u_mat, state0, cost0, t0, step_seconds, terminal_soc_target=None, terminal_tmedian_target=None):
...
for run_id in range(1, n_runs + 1):
    ...
    rf_model_soc = RandomForestRegressor(n_estimators=200, random_state=111)  # Use Random Forest
    rf_model_tmedian = RandomForestRegressor(n_estimators=200, random_state=111)  # Use Random Forest
    clf_features = []
    terminal_soc_values = []
    terminal_tmedian_values = []


    exploration_decision_log = []
    exploration_decision_file = os.path.join(run_results_dir, "exploration_decisions.txt")
    seasonal_warmup_steps = 10

    with open(exploration_decision_file, "w") as f:
        f.write("current_step\tforce_exploration\treason\tmean_soc_prediction\tterminal_soc\tucb_soc\tmean_tmedian_prediction\tterminal_tmedian\tucb_tmedian\tbudget\n")

    while current_step < total_steps:
        current_price_seq = get_24h_price_projection(current_step)

        # Init or price change = boosted evals
        if last_price_seq is None or not np.allclose(current_price_seq, last_price_seq) or current_step < BLOCK:
            if last_price_seq is not None:
                print(f"Price profile changed at step {current_step} → reset warm start")
                elite_pool = np.empty((0, 2*horizon))
            seasonal_warmup_steps = 10 #int((10 * 24 * 3600) / STEPS_SECONDS)  # 10-day warmup
        ...
        feat_vec = get_48h_weather_projection(current_step)

        print('feat vec size', len(feat_vec))

        force_exploration = False
        terminal_soc = None
        terminal_tmedian = None
        mean_soc_prediction = None
        mean_tmedian_prediction = None
        std_soc_prediction = None
        std_tmedian_prediction = None
        ucb_soc = None
        ucb_tmedian = None
        reason = ""
        ...
        if steps_left <= BLOCK:
            force_exploration = False
            reason = "final_day_no_exploration"
            terminal_soc = 0.0
            terminal_tmedian = None
            print("Final day of simulation.")

        elif seasonal_warmup_steps > 0 and current_step > BLOCK:
            force_exploration = True
            reason = "seasonal_warmup"
            seasonal_warmup_steps -= 1
            if len(clf_features) > 0:
                if len(feat_vec) != clf_features[0].shape[0]:
                    prediction_log.append((current_step, None, None))
                    force_exploration = False
                    reason = "missing_features"
                    terminal_soc = 0.0
                    
        elif (current_step // (24 * 3600 / STEPS_SECONDS)) % 5 == 0 and current_step > BLOCK:
            force_exploration = True
            reason = "periodic_rule"
            print("Exploration on! Gathering data")
        

        elif current_step < BLOCK:
            force_exploration = False
            reason = "first_day_skip"
            print("Skipping first day due to system warmup!")
            terminal_soc = 0
            terminal_tmedian = None

                    
        else:
            if len(clf_features) >= seasonal_warmup_steps:
                rf_model_soc.fit(np.array(clf_features), terminal_soc_values)
                predictions_soc = np.array([tree.predict(feat_vec.reshape(1, -1)) 
                                         for tree in rf_model_soc.estimators_])
                mean_soc_prediction = np.mean(predictions_soc)
                std_soc_prediction = np.std(predictions_soc)
                ucb_soc = mean_soc_prediction + 2 * std_soc_prediction
                
                
                
                rf_model_tmedian.fit(np.array(clf_features), terminal_tmedian_values)
                predictions_tmedian = np.array([tree.predict(feat_vec.reshape(1, -1)) 
                                         for tree in rf_model_tmedian.estimators_])
                mean_tmedian_prediction = np.mean(predictions_tmedian)
                std_tmedian_prediction = np.std(predictions_tmedian)
                ucb_tmedian = mean_tmedian_prediction + 2 * std_tmedian_prediction
                
                
                
                prediction_log.append((current_step, mean_soc_prediction, std_soc_prediction, ucb_soc, mean_tmedian_prediction, std_tmedian_prediction, ucb_tmedian))
                print(f"Mean soc: {mean_soc_prediction:.6f}, Std soc: {std_soc_prediction:.6f}, UCB soc: {ucb_soc:.6f}, Mean tmedian: {mean_tmedian_prediction:.6f}, Std tmedian: {std_tmedian_prediction:.6f}, UCB tmedian: {ucb_tmedian:.6f}")
            
                pred_file = os.path.join(run_results_dir, "exploration_predictions.txt")
                with open(pred_file, 'a') as f:
                    f.write(f"{current_step}\t{mean_soc_prediction}\t{std_soc_prediction}\t{ucb_soc}\t{mean_tmedian_prediction}\t{std_tmedian_prediction}\t{ucb_tmedian}\n")
                if ucb_soc >= 0.01 or ucb_tmedian >= 344.15:
                    force_exploration = True
                    reason = "RF_high_ucb"
                else:
                    force_exploration = False
                    reason = "RF_confident_skip"
                    terminal_soc = mean_soc_prediction
                    terminal_tmedian = mean_tmedian_prediction

            else:
                prediction_log.append((current_step, None, None, None, None, None, None))
                force_exploration = True
                reason = "missing_features"
        ...
        if force_exploration:
            ...
            # Load state but handle FMICallException
            try:
                ...
                try:
                    ...
                    # Code to replay past controls...

            soc_term = fmu_term.getReal([vr_term['soc']])[0]
            steps_in_first_day = int((24 * 3600) / EXPL_STEP_SECONDS)
            ...
            clf_features.append(feat_vec)
            terminal_soc_values.append(terminal_soc)
            terminal_tmedian_values.append(terminal_tmedian)


        print('Terminal SOC:', terminal_soc)
        print('Terminal Tmedian:', terminal_tmedian)

        exploration_decision_log.append((current_step, force_exploration, reason))
        expl_budget_log = max_evals_expl if force_exploration else 0

        with open(exploration_decision_file, "a") as f:
            f.write(
                f"{current_step}\t"
                f"{int(force_exploration)}\t"
                f"{reason}\t"
                f"{-1 if mean_soc_prediction is None else mean_soc_prediction:.6f}\t"
                f"{-1 if terminal_soc is None else np.abs(terminal_soc):.6f}\t"
                f"{-1 if ucb_soc is None else ucb_soc:.6f}\t"
                f"{-1 if mean_tmedian_prediction is None else mean_tmedian_prediction:.6f}\t"
                f"{-1 if terminal_tmedian is None else np.abs(terminal_tmedian):.6f}\t"
                f"{-1 if ucb_tmedian is None else ucb_tmedian:.6f}\t"
                f"{expl_budget_log}\n"
            )
        ...
        def _one_cost_LF(x, state, c0, t0, term_soc, term_tmedian):
            try:
                u = np.column_stack((x[:H//2], x[H//2:]))
                return run_horizon(u, state, c0, t0, LF_STEP_SECONDS, term_soc, term_tmedian)
            except FMICallException:
                return 1e12
        ...
        def obj_batch_LF(X_batch):
            state_copy_list = [copy.deepcopy(STATE0) for _ in range(len(X_batch))]
            with mp.Pool(processes=os.cpu_count()) as pool:
                jobs = [(x, state_copy_list[i], cost0, T0_snap, terminal_soc, terminal_tmedian) for i, x in enumerate(X_batch)]
        ...
        def _one_cost_HF(x, state, c0, t0, term_soc, term_tmedian):
            try:
                u = np.column_stack((x[:H], x[H:]))
                return run_horizon(u, state, c0, t0, STEPS_SECONDS, term_soc, term_tmedian)
            except FMICallException:
                return 1e12

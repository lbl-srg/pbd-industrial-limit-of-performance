...
for run_id in range(1, n_runs + 1):
    ...
    gb_model_soc = GradientBoostingRegressor(random_state=111)
    gb_model_tmedian = GradientBoostingRegressor(random_state=111)
    
    gb_perc_model_soc = GradientBoostingRegressor(loss='quantile', alpha=0.95)
    gb_perc_model_tmedian = GradientBoostingRegressor(loss='quantile', alpha=0.95)
    ...
    while current_step < total_steps:
        ...
        if steps_left <= BLOCK:
            ...
            if len(clf_features) >= seasonal_warmup_steps:
                gb_model_soc.fit(np.array(clf_features), terminal_soc_values)
                gb_perc_model_soc.fit(np.array(clf_features), terminal_soc_values)

                mean_soc_prediction =  gb_model_soc.predict(feat_vec.reshape(1, -1))[0]
                ucb_soc =  gb_perc_model_soc.predict(feat_vec.reshape(1, -1))[0]
                
                gb_model_tmedian.fit(np.array(clf_features), terminal_tmedian_values)
                gb_perc_model_tmedian.fit(np.array(clf_features), terminal_tmedian_values)

                mean_tmedian_prediction =  gb_model_tmedian.predict(feat_vec.reshape(1, -1))[0]
                ucb_tmedian =  gb_perc_model_tmedian.predict(feat_vec.reshape(1, -1))[0]
                
                
                prediction_log.append((current_step, mean_soc_prediction,  ucb_soc, mean_tmedian_prediction, ucb_tmedian))
                print(f"Mean soc: {mean_soc_prediction:.6f}, UCB soc: {ucb_soc:.6f}, Mean tmedian: {mean_tmedian_prediction:.6f}, UCB tmedian: {ucb_tmedian:.6f}")
            
                pred_file = os.path.join(run_results_dir, "exploration_predictions.txt")
                with open(pred_file, 'a') as f:
                    f.write(f"{current_step}\t{mean_soc_prediction}\t{ucb_soc}\t{mean_tmedian_prediction}\t{ucb_tmedian}\n")
                if ucb_soc >= 0.01 or ucb_tmedian >= 344.15:
                    force_exploration = True
                    reason = "GB_high_ucb"
                else:
                    force_exploration = False
                    reason = "GB_confident_skip"
                    terminal_soc = mean_soc_prediction
                    terminal_tmedian = mean_tmedian_prediction
            else:
                prediction_log.append((current_step, None, None, None, None))
                force_exploration = True
                reason = "missing_features"

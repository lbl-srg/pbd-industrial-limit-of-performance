...
class ElectricalSplitterScaler(CustomScalerBase):
    ...
    def variable_scaling_routine(
        ...
        self.set_variable_scaling_factor(
            model.electricity[0], 1e-5, overwrite=overwrite
        )
        for p in model.outlet_list:
            out_port = getattr(model, p + "_elec")
            self.set_variable_scaling_factor(
                out_port[0],
...
@declare_process_block_class(
    "ElectricalSplitter", doc="Splits electricity flow into outlet stream"
)
class ElectricalSplitterData(UnitModelBlockData):
    ...
    default = ElectricalSplitterScaler
    ...
    def build(self):
        ...
        def sum_split(b, t):
            return b.electricity[t] == sum(
                getattr(b, o + "_elec")[t] for o in b.outlet_list
            )
    ...
    def initialize_build(self, **kwargs):
        ...
        if self.config.add_split_fraction_vars:
            outlet_vars = [
                Reference(self.split_fraction[o, :]) for o in self.outlet_list
            ]
        else:
    ...
    def report(self, time_point=0, dof=False, ostream=None, prefix=""):
        ...
        stream_attributes["Inlet"] = {
            "electricity": value(self.electricity[time_point])
        }
        stream_attributes["Outlet"] = {}
        for outlet in self.outlet_list:
            stream_attributes["Outlet"][outlet] = value(
                getattr(self, outlet + "_elec")[time_point]
            )
